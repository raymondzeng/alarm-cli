...
# def do_query(query, songs, artists, albums, library):
...
#    playlists   =   kwargs['playlists']
 ...
 #   quiet       =   kwargs['quiet']
    ...
    if not query:
        ...
        query = ' '.join(query)
     #   matches = do_query(query, songs, artists, albums, library)
        ...
        if len(results) >= 1:
            ...
            if index is not None:
                ...
                # removed album col, changed *3 to * 2
                fmt = ('| {{:<7}} | ' + '{{:<{}}} | '*2).format(*longest)
                print "More than one result. " \
                      "(Try a better query, or specify an index using -i/--index):"
                ...
                for (i, song) in enumerate(results):
                 ...
                 #   if len(album)  > longest[2]: album  = album[:longest[2]-3]  + '...'
                    ...
                    print fmt.format(i, name.encode('utf-8'), artist.encode('utf-8')) #, album)
